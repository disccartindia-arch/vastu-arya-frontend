    ...
    <>
      ...
      <div className="text-white text-xs py-1.5 px-4 flex items-center justify-between" style={{ background: '#0D0500' }}>
        <div className="flex items-center gap-4">
          <a href={`tel:${brand.phone}`} className="hover:text-yellow-400 transition-colors" style={{ color: '#D4A017' }}>
            {brand.phone}
          </a>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 gap-3">
            ...
            <Link href="/" className="flex items-center gap-3 group flex-shrink-0">
              <div
                className="w-11 h-11 rounded-full overflow-hidden flex-shrink-0 transition-transform group-hover:scale-105"
            ...
            <div className="hidden xl:flex items-center gap-0.5 flex-1 justify-center">
              {NAV_LINKS.map(link => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-2.5 py-2 text-sm font-medium rounded-lg transition-colors whitespace-nowrap hover:text-primary hover:bg-orange-50"
                  style={{ color: '#5C3D1E' }}
                >
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              {user ? (
                <div className="relative group">
                  ...
                  <div
                    className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-xl py-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50"
                    style={{ border: '1px solid rgba(212,160,23,0.15)' }}
                  >
                    {isAdmin() && (
                      <Link href="/admin" className="block px-4 py-2 text-sm font-semibold hover:bg-orange-50 transition-colors" style={{ color: '#FF6B00' }}>
                        Admin Panel
                      </Link>
                    )}
                    <Link href="/dashboard" className="block px-4 py-2 text-sm hover:bg-orange-50 transition-colors" style={{ color: '#5C3D1E' }}>
                      My Account
                    </Link>
                    <button onClick={logout} className="block w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition-colors">
                      Logout
                    </button>
                  </div>
                </div>
              ) : (
                <Link href="/login" className="hidden sm:block text-sm font-medium hover:text-primary transition-colors" style={{ color: '#5C3D1E' }}>
                  Login
                </Link>
              )}
          ...
          <div className="border-t border-orange-100 px-4 py-3" style={{ background: 'rgba(255,255,255,0.98)' }}>
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex gap-2">
              <input
                ...
                placeholder="Search services, blogs, products, Vastu tips…"
                className="flex-1 px-4 py-2.5 text-sm border border-orange-200 rounded-xl focus:outline-none focus:border-primary"
              />
              <button
                type="submit"
          ...
          <div className="xl:hidden border-t bg-white px-4 py-4 space-y-1" style={{ borderColor: 'rgba(212,160,23,0.1)' }}>
            ...
            <form onSubmit={handleSearch} className="flex gap-2 mb-3">
              <input
                ...
                onChange={e => setSearchQuery(e.target.value)}
                placeholder="Search…"
                className="flex-1 px-3 py-2 text-sm border border-orange-200 rounded-xl focus:outline-none focus:border-primary"
              />
              <button type="submit" className="px-3 py-2 bg-primary text-white rounded-xl text-sm">
                <Search size={14} />
              </button>
            </form>
            ...
            <button
              onClick={() => { setShowAppointmentPopup(true); setMobileMenuOpen(false); }}
