...
import { Sparkles, Home, Phone, MessageCircle, Calendar, CheckCircle, Star, Send, X, Shield, Clock } from 'lucide-react';
...
function ResultsModal({ result, isDemo, concern, onClose, onBook }: {
  result: AIResult; isDemo: boolean; concern: string; onClose: () => void; onBook: () => void;
}) {
  const waMsg = encodeURIComponent(`Namaste Dr. Pranoy ji, I received AI Vastu guidance for "${concern.slice(0,50)}" and would like a personal consultation.`);
  return (
    <AnimatePresence>
        ...
        <motion.div initial={{opacity:0,y:60,scale:0.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:60}}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-orange-200 flex-shrink-0">
                  <img src="/logo.jpg" alt="Dr. Pranoy" className="w-full h-full object-cover"/>
                </div>
                <div>
                  <p className="font-bold text-gray-800 text-sm">Dr. Pranoy Pratap Singh Tomar</p>
                  <div className="flex items-center gap-0.5">{[...Array(5)].map((_,i)=><Star key={i} size={10} className="fill-yellow-400 text-yellow-400"/>)}<span className="text-xs text-gray-400 ml-1">IVAF Certified</span></div>
                </div>
              </div>
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 transition-colors"><X size={18} className="text-gray-400"/></button>
            </div>
            {isDemo && <div className="mt-2 text-xs bg-amber-50 border border-amber-200 rounded-xl px-3 py-1.5 text-amber-700">⚡ Demo mode — Add ANTHROPIC_API_KEY to Render env for live AI</div>}
          </div>
          <div className="px-5 py-4 space-y-4">
            ...
            {result.remedies?.length > 0 && (
              <div>
                <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3"><CheckCircle size={18} className="text-green-500"/>Your Personalised Vastu Remedies</h3>
                  ...
                  {result.remedies.map((r,i)=>(
                    <motion.div key={i} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{delay:i*0.08}}
                      ...
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0 mt-0.5"
                        style={{background:'linear-gradient(135deg,#FF6B00,#FF9933)'}}>{i+1}</div>
                      <div className="flex-1">
                        <p className="font-bold text-gray-800 text-sm mb-1">{r.title}</p>
                        <p className="text-gray-600 text-xs mb-1.5">✅ {r.action}</p>
                        <div className="flex flex-wrap gap-2 text-xs text-gray-400"><span>📍 {r.zone}</span><span>💡 {r.benefit}</span></div>
                      </div>
                    </motion.div>
                  ))}
                </div>
            ...
            <div className="rounded-2xl p-4 border" style={{background:'linear-gradient(135deg,#0D0500,#1A0A00)',borderColor:'rgba(212,160,23,0.3)'}}>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 rounded-full overflow-hidden border-2 flex-shrink-0" style={{borderColor:'rgba(212,160,23,0.5)'}}>
                  <img src="/logo.jpg" alt="Dr. Pranoy" className="w-full h-full object-cover"/>
                </div>
                <div>
                  <p className="font-bold text-white text-sm">Want a Deeper Personal Analysis?</p>
                  <p className="text-xs mt-0.5" style={{color:'#D4A017'}}>Dr. Pranoy Pratap Singh Tomar • IVAF Certified • 45,000+ Clients</p>
                </div>
              </div>
              <p className="text-gray-400 text-xs leading-relaxed mb-3">{result.consultationCTA} A personal consultation analyses your exact floor plan and birth chart for maximum accuracy.</p>
              <div className="flex gap-2 mb-2 text-xs" style={{color:'#D4A017'}}>
                {[{icon:Shield,t:'100% Authentic'},{icon:Clock,t:'15+ Years'},{icon:Star,t:'IVAF Certified'}].map(({icon:Icon,t})=>(
                  <span key={t} className="flex items-center gap-1"><Icon size={10}/>{t}</span>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-2 mb-2">
                <button onClick={onBook} className="flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-sm text-white"
              ...
              <a href={`tel:+${WA}`} className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm border w-full hover:bg-white/5 transition-all"
                style={{borderColor:'rgba(212,160,23,0.3)',color:'#D4A017'}}>
                <Phone size={13}/> Call Dr. Pranoy Directly
              </a>
            </div>
            <button onClick={onClose} className="w-full py-3 rounded-xl border border-orange-200 text-primary font-medium text-sm hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2">
...
export default function VastuAIPage() {
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      if (r?.data?.success) { setResult(r.data.data); setIsDemo(r.data.isDemo || false); }
      else toast.error(r?.data?.message || 'Analysis failed. Please try again.');
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'AI analysis unavailable. Please try again in a moment.');
  ...
  return (
    <>
      ...
      <main style={{background:'linear-gradient(135deg,#FFFDF7 0%,#FFF8EE 100%)',minHeight:'100vh'}}>
        <section className="py-16 text-center relative overflow-hidden" style={{background:'linear-gradient(135deg,#0D0500,#1A0A00)'}}>
          ...
          <div className="relative max-w-3xl mx-auto px-4">
            ...
            <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.2}} className="text-gray-300 text-lg">
              Describe your concern and get personalised remedies — powered by Dr. Pranoy&apos;s 15 years of expertise.
            </motion.p>
          </div>
        </section>
        ...
        <div className="max-w-2xl mx-auto px-4 py-12">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-3xl p-6 sm:p-8 shadow-lg border border-orange-100">
            ...
            <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-2xl border border-orange-100 mb-6">
              ...
              <div className="flex items-center gap-1 text-xs font-semibold text-green-600 bg-green-50 px-2.5 py-1 rounded-full border border-green-200 flex-shrink-0">
                <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"/><span>Online</span>
              </div>
            </div>
            {/* Concern chips */}
            <div className="mb-4">
              <p className="text-xs text-gray-400 font-medium mb-2 flex items-center gap-1"><Home size={11}/>Common concerns — tap to add:</p>
              <div className="flex flex-wrap gap-2">
                {concerns.map(c=>(
                  <button key={c} onClick={()=>addConcern(c)}
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-6">
              <div>
                ...
                <select value={roomType} onChange={e=>setRoomType(e.target.value)} className="w-full px-3 py-2.5 border border-orange-200 rounded-xl text-sm focus:outline-none focus:border-primary bg-white">
              ...
              <div>
                ...
                <select value={direction} onChange={e=>setDirection(e.target.value)} className="w-full px-3 py-2.5 border border-orange-200 rounded-xl text-sm focus:outline-none focus:border-primary bg-white">
            ...
            <button onClick={handleAnalyse} disabled={loading||concern.trim().length<10}
              ...
              style={{background:'linear-gradient(135deg,#FF6B00,#FF9933)',boxShadow:concern.trim().length>=10?'0 8px 24px rgba(255,107,0,0.35)':'none'}}>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            {[{step:'1',title:'Describe',desc:'Share your concern'},{step:'2',title:'AI Analyses',desc:'Get remedies instantly'},{step:'3',title:'Apply & Book',desc:'Transform your space'}].map(s=>(
              <div key={s.step} className="text-center">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm mx-auto mb-2"
                  style={{background:'linear-gradient(135deg,#FF6B00,#FF9933)'}}>{s.step}</div>
      ...
      {result && (
        <ResultsModal result={result} isDemo={isDemo} concern={concern}
