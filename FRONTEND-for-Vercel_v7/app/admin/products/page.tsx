...
export default function AdminProductsPage() {
  ...
  const load = (cat?: string) => {
    ...
    setLoading(true);
    productsAPI.getAdminAll(params)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!payload.slug) payload.slug = payload.name.en.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      if (editing) { await productsAPI.update(editing._id, payload); toast.success('Product updated!'); }
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${!catFilter ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
        >All</button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>{['Product','Category','Price','Offer','Stock','Status','Actions'].map(h => (
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-cream rounded-lg flex-shrink-0 overflow-hidden">
                          {p.images[0]
                            ? <img src={p.images[0]} alt={p.name.en} className="w-full h-full object-cover" />
                            : <span className="w-full h-full flex items-center justify-center text-lg">🕉️</span>
                          }
                        </div>
                        <div>
                          <p className="font-medium text-sm text-gray-800 line-clamp-1">{p.name.en}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium w-fit ${p.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
            ...
            {products.length === 0 && (
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="text-xs text-gray-300">Click <strong className="text-green-600">🌱 Seed 45 Products</strong> to auto-populate all categories.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="label-style">Name (English) *</label>
                  <input value={form.name.en} onChange={e => setForm({ ...form, name: { ...form.name, en: e.target.value } })} className="input-style w-full" placeholder="Product name" />
                </div>
                <div>
                  <label className="label-style">Name (Hindi)</label>
                  <input value={form.name.hi} onChange={e => setForm({ ...form, name: { ...form.name, hi: e.target.value } })} className="input-style w-full font-hindi" placeholder="हिंदी नाम" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })} className="input-style w-full" placeholder="VA-DF-001" />
                </div>
                <div>
                  <label className="label-style">Original Price (₹) *</label>
                  <input type="number" value={form.price} onChange={e => setForm({ ...form, price: +e.target.value })} className="input-style w-full" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" value={form.stock} onChange={e => setForm({ ...form, stock: +e.target.value })} className="input-style w-full" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="input-style w-full" rows={3}
                />
              </div>

              {/* ── IMAGE UPLOADER (replaces URL text inputs) ── */}
              <ImageUploader
                images={form.images}
                ...
                {form.benefits.map((b: string, i: number) => (
                  <input
                    ...
                    onChange={e => {
                      ...
                      setForm({ ...form, benefits: bens });
                    }}
                    className="input-style w-full mb-2"
                    placeholder={`Benefit ${i + 1} (e.g. Removes Vastu doshas)`}
                  />
                ))}
                <button
                  type="button"
              ...
              <div className="flex gap-6">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isNewLaunch} onChange={e => setForm({ ...form, isNewLaunch: e.target.checked })} className="w-4 h-4 accent-primary" />
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex gap-3">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary-dark disabled:opacity-60 flex items-center justify-center gap-2"
              >
