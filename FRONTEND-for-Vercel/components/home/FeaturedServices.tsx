...
interface Props { services: Service[]; onBookAppointment: () => void; }

export default function FeaturedServices({ services, onBookAppointment }: Props) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mt-10">
          <Link href="/services" className="inline-flex items-center gap-2 border-2 border-primary text-primary hover:bg-primary hover:text-white px-8 py-3 rounded-xl font-semibold transition-all">
            {lang === 'en' ? 'View All 100+ Services' : 'सभी 100+ सेवाएं देखें'} <ArrowRight size={18} />
          </Link>
        </div>
      </div>
    </section>
