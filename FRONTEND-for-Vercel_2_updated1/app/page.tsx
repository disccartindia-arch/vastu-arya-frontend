...
import { servicesAPI, productsAPI, contentAPI, homepageSettingsAPI } from '../lib/api';
...
export default function HomePage() {
  const { lang, setShowAppointmentPopup, setContactNumber } = useUIStore();
  ...
  useEffect(() => {
    ...
    ]).then(([sRes, pRes, cRes, hRes]) => {
      setServices(sRes.data.data || []);
      setProducts(pRes.data.data || []);
      setCms((cRes as any).data?.data || {});

      const hs = hRes?.data?.data;
      if (hs) {
        setHeroSettings({
        ...
        if (hs.contactNumber) setContactNumber(hs.contactNumber);
      }
  ...
  // Stats from heroSettings (admin-configured) or CMS fallback
  ...
  return (
    <>
      <Navbar />
      <main>
        <HeroSection onBookClick={() => setShowAppointmentPopup(true)} settings={heroSettings} />
