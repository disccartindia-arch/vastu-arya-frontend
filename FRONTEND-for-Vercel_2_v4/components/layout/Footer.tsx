...
export default function Footer() {
  ...
  // Footer content from admin Website Editor → Global → Footer & Contact
  const [fd, setFd] = useState<Record<string, string>>({});

  useEffect(() => {
    contentAPI.getPage('global')
      .then((r: any) => {
        const d = r?.data?.data;
        if (d?.footer) setFd(d.footer);
      })
      .catch(() => {});
  }, []);

  // Helper with fallback
  const f = (key: string, fallback: string) => fd[key] || fallback;

  const phone     = f('phone', '+91-9999999999');
  const email     = f('email', 'contact@vastuarya.com');
  const address   = f('address', 'New Delhi, India');
  const whatsapp  = f('whatsapp', '919999999999');
  const copyright = f('copyright', '© 2024 Vastu Arya | IVAF Certified | Made with love in India');
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-gray-600">
            {lang === 'hi' ? '© 2024 वास्तु आर्या | IVAF प्रमाणित | भारत में बना' : copyright}
          </p>
