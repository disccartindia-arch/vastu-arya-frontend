...
// ── Updated SECTIONS — expanded footer + new testimonials section ─────────────
const SECTIONS: Record<string, any[]> = {
  home: [
    ...
    { key: 'stats', label: 'Statistics (CMS fallback)', fields: [
    ...
    { key: 'testimonials', label: 'Testimonials Section', fields: [
      { key: 'title', label: 'Section Title (e.g. What Our Clients Say)' },
      { key: 'subtitle', label: 'Clients Count Text (e.g. 45,000+ happy clients across India)' },
    ]},
  ...
  global: [
    ...
    // ── Expanded footer section — all fields editable ─────────────────────────
    { key: 'footer', label: 'Footer & Contact Info', fields: [
      { key: 'tagline', label: 'Footer Tagline (below logo)' },
      { key: 'copyright', label: 'Copyright Text (bottom bar)' },
      { key: 'phone', label: 'Contact Phone Number' },
      { key: 'email', label: 'Contact Email Address' },
      { key: 'address', label: 'Office Address' },
      { key: 'whatsapp', label: 'WhatsApp Number (with country code, e.g. 919999999999)' },
...
export default function WebsiteEditorPage() {
  ...
  const TABS = [
    { id: 'content' as const, label: 'Content Editor', icon: Type },
    { id: 'background' as const, label: 'Background & Animation', icon: Zap },
    { id: 'logo' as const, label: 'Logo & Branding', icon: Image },
  ];
    ...
    <div className="space-y-5">
              ...
              sections.map((sec: any) => (
                <div key={sec.key} className="bg-white rounded-2xl p-5 shadow-sm">
                  <h3 className="font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100 flex items-center gap-2">
                    {sec.label}
                    {sec.key === 'footer' && (
                      <span className="text-xs bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full">Editable footer details</span>
                    )}
                    {sec.key === 'testimonials' && (
                      <span className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">Shown above testimonial cards</span>
                    )}
                  </h3>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {sec.fields.map((f: any) => (
                      <div key={f.key} className={f.label.length > 40 ? 'sm:col-span-2' : ''}>
                        ...
                        <input value={content[sec.key]?.[f.key] || ''} onChange={e => updateContent(sec.key, f.key, e.target.value)}
                          className="input-style" placeholder={`Enter ${f.label.split('(')[0].trim()}...`} />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {activeTab === 'background' && (
        <div className="bg-white rounded-2xl p-6 shadow-sm max-w-2xl space-y-4">
          ...
          {/* ── Hero Background Theme (new) ── */}
          <div>
            <h3 className="font-semibold text-gray-800 pb-2 border-b border-gray-100 mb-4">
              Hero Section Background Theme
            </h3>
            <p className="text-xs text-gray-500 mb-3">Controls the homepage hero section background style. Changes apply on next page load.</p>
            <div className="grid grid-cols-2 gap-3 mb-2">
              {[
                { value: 'light', emoji: '☀️', label: 'Warm Light', desc: 'Rich cream / white background (recommended)' },
                ...
                <button key={opt.value}
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-all ${
                    (config.hero_bg_theme || 'light') === opt.value
              ...
              {f.type === 'bool' ? (
                ...
                <div className="flex items-center gap-2">
                  <input type="range" min={f.min} max={f.max} step={f.step}
                    value={config[f.key] ?? (((f.max as number) + (f.min as number)) / 2)}
          ...
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-xl text-xs text-yellow-700">
            💡 Background animations are GPU-optimized. Hero theme changes apply on next page load.
          </div>
        </div>
      )}
